...
#define TEST_DESC "Verify same priority events run in sequence    "
...
static uint64_t second_event_register(struct sdei_event *second_event)
{
    ...
    /* Bind second interrupt to event */
...
SDEI_SET_TEST_DEPS(test_040_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_040, TEST_040_ID, TEST_DESC, test_040_deps, test_entry, FALSE);
