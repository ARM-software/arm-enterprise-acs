...
#define TEST_DESC "Verify always availability- INTERRUPT_BIND     "
...
static uint32_t g_event_num;

/* Check the SDEI_INTERRUPT_BIND API in below states
...
static void event_handler(void)
{
    ...
    err = val_sdei_interrupt_bind(SPI_INTR_NUM1, &g_event_num);
    if (err) {
...
static void test_entry(void) {
    ...
    do {
        ...
        err = val_gic_disable_interrupt(SPI_INTR_NUM1);
        if (err) {
        ...
        err = val_sdei_interrupt_bind(SPI_INTR_NUM1, &g_event_num);
        if (err) {
            val_print(ACS_LOG_ERR, "\n        SPI intr number %d bind failed with err %d",
                                                                                SPI_INTR_NUM1, err);
            g_test_status = SDEI_TEST_FAIL;
            goto event_unregister;
        }
        val_pe_data_cache_clean_invalidate((uint64_t)&g_event_num);
...
interrupt_release:
    err = val_sdei_interrupt_release(g_event_num);
    if (err) {
        val_print(ACS_LOG_ERR, "\n        Event num %d release failed : err %d",
                                                            g_event_num, err);
    }
...
SDEI_SET_TEST_DEPS(test_038_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_038, TEST_038_ID, TEST_DESC, test_038_deps, test_entry, FALSE);
