...
#define TEST_DESC "Verify SDEI event disable test                  "

#define SDEI_UNKNOWN_EVENT 0x10000
#define ENABLED 0x2

/* This function verifies the shared and private events disable functionality.
 * it also checks event disable error checks.
 */
static uint32_t payload(void *data)
{
    ...
    /*Try to to disable unregistered event */
    err = val_sdei_event_disable(event_num);
...
SDEI_SET_TEST_DEPS(test_006_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_006, TEST_006_ID, TEST_DESC, test_006_deps, test_entry, TRUE);
