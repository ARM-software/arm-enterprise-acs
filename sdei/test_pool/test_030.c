...
#define TEST_DESC "Verify events are higher priority than intr    "
...
static volatile uint64_t g_status = SDEI_TEST_FAIL;

/* Generate Event in interrupt handler to check the preemption */
...
static void event_handler(void)
{
    ...
    /* If event is preempted interrupt handler, assign status to TEST_PASS */
    if (g_handler_flag)
        g_status = SDEI_TEST_PASS;
    g_handler_counter++;
}
...
SDEI_SET_TEST_DEPS(test_030_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_030, TEST_030_ID, TEST_DESC, test_030_deps, test_entry, FALSE);
