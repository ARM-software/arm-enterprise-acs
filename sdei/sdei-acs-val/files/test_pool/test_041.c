...
static void second_event_handler(void)
{
    ...
    /* If Second event is preempted WD event handler, assign status to TEST_FAIL */
    if (g_handler_flag)
...
static void test_entry(void) {
    ...
    while (timeout--) {
        ...
        if ((g_handler_flag == 0) && (g_event_counter == 2))
            break;
    }
    if (g_handler_flag || (g_event_counter != 2)) {
...
SDEI_SET_TEST_DEPS(test_041_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_041, TEST_041_ID, TEST_DESC, test_041_deps, test_entry, FALSE);
