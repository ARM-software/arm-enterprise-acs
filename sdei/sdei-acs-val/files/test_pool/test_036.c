...
#define TEST_DESC "Verify always availability- PE_MASK            "
...
/* Check the SDEI_PE_MASK API in below states
...
static void test_entry(void)
{
    ...
    if ((result & EVENT_STATUS_REGISTER_BIT)) {
        ...
        if (err) {
            val_print(ACS_LOG_ERR, "\n        Evt num %d release failed : err %d",
                                                                    g_event.event_num, err);
            g_test_status = SDEI_TEST_ERROR;
        }
        goto unmap_va;
...
event_unregister:
    err = val_sdei_event_unregister(g_event.event_num);
    if (err) {
        val_print(ACS_LOG_ERR, "\n        SDEI evt %d unregister failed : err %x",
                                                                g_event.event_num, err);
    }
interrupt_release:
    err = val_sdei_interrupt_release(g_event.event_num);
...
SDEI_SET_TEST_DEPS(test_036_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_036, TEST_036_ID, TEST_DESC, test_036_deps, test_entry, FALSE);
