...
#define TEST_DESC "Verify mask status of the PE after handler     "
...
static void event_handler(void)
{
    ...
    if (err || (result != PE_MASKED)) {
        g_test_status = SDEI_TEST_FAIL;
    }
    g_handler_flag = 0;
}

static void unmask_cpu(void *ignored)
{
    val_sdei_unmask();
}
...
static void test_entry(void) {
    ...
    val_pe_execute_on_all(unmask_cpu, 0);
...
SDEI_SET_TEST_DEPS(test_046_deps, TEST_001_ID, TEST_002_ID);
SDEI_PUBLISH_TEST(test_046, TEST_046_ID, TEST_DESC, test_046_deps, test_entry, FALSE);
