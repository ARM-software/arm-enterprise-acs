...
/**
 * @brief   This API reads MPIDR system register and return the affx bits
 ...
 * @return  None
 */
void val_execute_on_pe(uint32_t index, void (*payload)(void), uint64_t test_input)
{
    ...
    do {
        ...
        /* Set the TEST function pointer in a shared memory location. This location
         * is read by the Secondary PE (val_test_entry()) and executes the test.
         */
    ...
    if (g_smc_args.Arg0 == (uint64_t)ARM_SMC_PSCI_RET_ALREADY_ON)
        ...
        if(g_smc_args.Arg0 == 0) {
            ...
            val_print(ACS_PRINT_ERR, "       PSCI_CPU_ON: failure  \n", 0);
    }
...
{
    val_pe_install_esr(EXCEPT_AARCH64_SYNC, esr);
}
