 ...
 * @return  32-bit data
 */
uint32_t val_gic_get_info(uint32_t type)
{
...
{
    return (32 * ((val_mmio_read(val_get_gicd_base() + 0x004) & 0x1F) + 1)-1);
...
{
    val_mmio_write(val_get_gicd_base() + 0x200 + ((intr_id >> 5) << 2), (1 << (intr_id % 32)));
}
#else
void val_gic_write_ispendreg(uint32_t intr_id)
{
