...
#define TEST_NUM   ACS_CACHE_TEST_NUM_BASE  +  4
#define TEST_DESC  "Check CCAP with CPOR Partitioning "
...
typedef struct {
    ...
    uint8_t cache_percent;           //as function of max (cpor&ccap) cache size
    uint8_t config_enable;
} test_config_t;

test_config_t test_config_data[] = {
    {"Latency check for 75% CCAP & 75% CPOR partition size", 75, 75, 75, TRUE},
    {"Latency check for 75% CCAP & 50% CPOR partition size", 75, 50, 75, FALSE},
    {"Latency check for 75% CCAP & 25% CPOR partition size", 75, 25, 75, TRUE}
};
...
static void payload()
{
    ...
    /*
     * Compute # of CCAP & CPOR supported MPAM caches nodes,
     * the max size of the cache node among them, and the
     * min partition id supported among all MPAM nodes
     */
    for (node_index = 0; node_index < total_nodes; node_index++) {

        if (val_cache_supports_ccap(node_index) && val_cache_supports_cpor(node_index)) {
            cache_maxsize = GET_MAX_VALUE(cache_maxsize, val_cache_get_size(node_index));
            ccap_cpor_nodes++;
        }
    ...
    /* Skip this test if no CCAP & CPOR supported MPAM nodes present in the system */
    if (ccap_cpor_nodes == 0) {
    ...
    for (index = 0; index < sizeof(test_config_data)/sizeof(test_config_t); index++) {

        if (test_config_data[index].config_enable) {
            /*
             * Loop over all cache nodes and configure nodes that support
             * both CCAP && CPOR as per configuration data, enable hard limiting
             * Disable partitioning for all other nodes that support CCAP or CPOR
             */
            for (node_index = 0; node_index < total_nodes; node_index++) {
                ...
                if (val_cache_supports_ccap(node_index) && val_cache_supports_cpor(node_index)) {
                    ...
                    val_cache_configure_ccap(node_index,
                                             minmax_partid,
                                             HARDLIMIT_EN,
                                             test_config_data[index].ccap_partition_percent
                                             );
                    /* Configure CPOR partitioning for min(max(PARTID)) */
                    val_cache_configure_cpor(node_index,
                                             minmax_partid,
                                             test_config_data[index].cpor_partition_percent
                                             );
                } else if (val_cache_supports_ccap(node_index)) {
                    /* Disable CCAP partitioning for min(max(PARTID)) */
                    val_cache_configure_ccap(node_index, minmax_partid, HARDLIMIT_DIS, 100);
                } else if (val_cache_supports_cpor(node_index)) {
            ...
            buf_size = cache_maxsize * test_config_data[index].cache_percent / 100 / 2;
            src_buf = val_allocate_buf(buf_size);
            dest_buf = val_allocate_buf(buf_size);

            val_print(ACS_PRINT_DEBUG, "\n     index               = %d\n", index);
            val_print(ACS_PRINT_DEBUG, "     cache_maxsize       = %d\n", cache_maxsize);
            val_print(ACS_PRINT_DEBUG, "     buf_size            = %d\n", buf_size);

            if ((src_buf == NULL) || (dest_buf == NULL)) {
                val_print(ACS_PRINT_ERR, "\n  Mem allocation for CCAP COPR buffers failed", 0x0);
