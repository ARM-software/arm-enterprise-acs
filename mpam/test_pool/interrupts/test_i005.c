...
#define TEST_NUM   ACS_INTR_TEST_NUM_BASE  +  5
#define TEST_DESC  "Check request PARTID out-of-range error"
...
static void payload()
{
    ...
    for (index = 0; index < total_nodes; index++) {
        ...
        status = val_node_generate_por_error(node_type, node_index);
        ...
        if (timeout == 0) {
            val_print(ACS_PRINT_ERR, "\n MSC POR Err Interrupt not received on %d   ", intr_num);
            val_set_status(pe_index, RESULT_FAIL(TEST_NUM, 02));
            return;
        }
